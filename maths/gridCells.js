createGrid = function({
    ...
    if(!res && !number) {
        res = [20,20];
        number = dimensions.map((dim, i)=>{ return Math.ceil(dim/res[i]) });
    } else if(res) {
        number = dimensions.map((dim, i)=>{ return Math.ceil(dim/res[i]) });
    } else if(!res) {
        res = dimensions.map((dim, i)=> { return dim/number[i] });
    }
    ...
    const self = {
        ...
        res,
        number,
        get dx() { return dimensions[0] },
        get dy() { return dimensions[1] },
        get resx() { return res[0] },
        get resy() { return res[1] },
        get numx() { return number[0] },
        get cols() { return number[0] },
        get numy() { return number[1] },
        get rows() { return number[1] }
    }
    ...
    createCell = function(pos) {
        return {
            pos,
            get row() {return pos[1]/res[1]},
            get col() {return pos[0]/res[0]},
            get y() {return pos[1]},
            get x() {return pos[0]},
            get i() {return pos[1]/res[1]},
            get j() {return pos[0]/res[0]},
            scl: dimensions,
            center: pos.map((item, i)=>{return item + res[i]/2}),
        }
    }
    ...
    self.cell = function(j,i) {
        if(j<0 || j>self.cols || i<0 || i>self.rows){ return null }
    ...
    self.setCellProp = function(name, callback = (/*cell*/) => {/*do something*/}){
